...
class TicketsController extends Controller
{
    ...
    public function create(Request $request): void
	{
		...
		redirect()->route('tickets.read', $id)->withToast('success', __('ticket_open'))->go();
	}
    ...
    public function createMessage(Request $request, TicketMessages $messages): void
	{
		...
		redirect()->route('tickets.read', $request->ticket_id)->withToast('success', __('message_sent'))->go();
	}
	...
	public function delete(Request $request, ?int $id = null): void
	{
		...
		if (!is_null($id)) {
            ...
            redirect()->route('tickets.index', Auth::get('id'))->withToast('success', __('ticket_deleted'))->go();
		} else {
            Activity::log(__('tickets_deleted'));
            Alert::toast(__('tickets_deleted'))->success();
            response()->json(['redirect' => route('tickets.index', Auth::get('id'))]);
        }
	}
}
